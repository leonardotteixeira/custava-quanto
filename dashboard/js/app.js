...
function renderPrice(animate = true) {
  ...
  const anns = [
    { r: first, sub: mesAnoCurto(first.ano_mes), place: "above" },
    { r: last, sub: mesAnoCurto(last.ano_mes), signal: true },
  ];
  const far = (r) => anns.every((a) => Math.abs(monthIdx(a.r.ano_mes) - monthIdx(r.ano_mes)) > 5);
  if (far(hi)) anns.push({ r: hi, sub: `máxima · ${mesAnoCurto(hi.ano_mes)}` });
  if (far(lo)) anns.push({ r: lo, sub: `mínima · ${mesAnoCurto(lo.ano_mes)}`, place: "below" });
  ...
  lineChart($("#main-chart"), {
    series: [
      { rows: rows.map((r) => ({ iso: r.ano_mes, v: get(r) })), area: true },
      ...(ref ? [{ rows: rows.map((r) => ({ iso: r.ano_mes, v: ref(r) })), cls: "c-line c-line--ref" }] : []),
    ],
    ...
    tooltip: (mi) => {
      ...
      if (!r) return "";
      const lines = [`<div class="t-when">${mesAno(r.ano_mes)} · governo ${r.periodo}</div><div class="t-val">${vFmt(get(r))}</div>`];
...
function renderContext() {
  ...
  if (k === "taxa") {
    const compGet = code === "SELIC" ? (r) => r.ipca_var_12m : (r) => r.selic_meta_aa;
    const compName = code === "SELIC" ? "Inflação em 12 meses (IPCA)" : "Selic";
    const rows = prod.serie_mensal;
    $("#context-deck").textContent = "Selic e inflação em 12 meses usam a mesma unidade (% ao ano), então dividem o mesmo eixo, sem precisar de base 100. Quando a linha da Selic está acima da outra, os juros superam a inflação.";
    box.style.setProperty("--cols", 1);
    box.innerHTML = `<div class="mult overlay"><div class="legend"><span><i class="k"></i>${m.titulo === "Taxa Selic" ? "Selic" : "Inflação em 12 meses (IPCA)"}</span><span><i class="k k--dot" style="border-color:var(--fg-3)"></i>${compName}</span></div><div class="chart" id="ctx-overlay" tabindex="0" role="img"></div></div>`;
    const lp = lastValid(rows, (r) => r.taxa_aa), lc = lastValid(rows, compGet);
    lineChart($("#ctx-overlay"), {
      series: [
        { rows: rows.map((r) => ({ iso: r.ano_mes, v: r.taxa_aa })) },
        { rows: rows.map((r) => ({ iso: r.ano_mes, v: compGet(r) })), cls: "c-line c-line--ref" },
      ],
      cutoff: PERIODO_CORTE, bands: "full", includeZero: true, yFmt: (t) => `${fmtNum(t, 0)}%`, marginRight: 8,
      annotations: [
        { iso: lp.ano_mes, v: lp.taxa_aa, text: `${code === "SELIC" ? "Selic" : "IPCA"} ${fmtNum(lp.taxa_aa, 2)}%`, signal: true },
        { iso: lc.ano_mes, v: compGet(lc), text: `${code === "SELIC" ? "IPCA" : "Selic"} ${fmtNum(compGet(lc), 2)}%`, place: "below" },
      ],
      tooltip: (mi) => {
        const r = rows.find((q) => monthIdx(q.ano_mes) === mi);
        return r ? `<div class="t-when">${mesAno(r.ano_mes)}</div><div class="t-row"><span><i class="t-key" style="border-color:var(--on-night)"></i>${code === "SELIC" ? "Selic" : "IPCA 12m"}</span><b>${fmtNum(r.taxa_aa, 2)}%</b></div><div class="t-row"><span><i class="t-key" style="border-color:var(--on-night-3);border-top-style:dotted"></i>${code === "SELIC" ? "IPCA 12m" : "Selic"}</span><b>${isNil(compGet(r)) ? "—" : fmtNum(compGet(r), 2) + "%"}</b></div>` : "";
      },
    });
    $("#ctx-overlay").setAttribute("aria-label", `${m.titulo} e ${compName}, mês a mês, em % ao ano. Último mês: ${fmtNum(lp.taxa_aa, 2)}% e ${fmtNum(compGet(lc), 2)}%.`);
...
function tmInstruments() {
  ...
  return [...TM_FIXED.slice(0, 3), "SALARIO", ...TM_FIXED.slice(4), slot].map((c) => {
    ...
    const label = c === "IPCA" ? "Inflação · 12 meses" : c === "SELIC" ? "Selic · % ao ano" : c === "IBOVESPA" ? "Ibovespa · pontos" : c === "DOLAR" ? "Dólar · R$" : k === "indice" ? `${META[c].curto} · índice` : `${META[c].curto} · R$${c === "GLP" ? "/botijão" : "/litro"}`;
    const html = k === "pontos" ? (v) => `${fmtNum(v / 1000, 1)}<span class="suf">mil</span>` : (v) => fmtValueHTML(prod, v);
    return { code: c, label, prod, get: nativeValue(prod), fmt: (v) => fmtValue(prod, v), html, product: c === S.product };
  });
}
...
function renderArchive() {
  ...
  const anVal = (r) => (!r ? null : k === "preco" ? r.preco_nominal_medio : k === "indice" ? r.indice_nominal_medio : k === "taxa" ? r.taxa_media : r.pontos_medio);
