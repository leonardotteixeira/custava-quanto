...
export function comparisonPoints(prod, base) {
  ...
  // PIB é anual: cada ponto é um ano ("2022"), não um mês.
  if (prod.tipo === "pib") return { a: { row: ra, iso: ra.ano_mes, v: get(ra), daily: false, label: String(ra.ano) }, b: { row: rb, iso: rb.ano_mes, v: get(rb), daily: false, label: String(rb.ano) } };
  const d = prod.diario;
...
export const pointLabel = (pt) => pt.label ?? (pt.daily ? dataCurta(pt.iso) : mesAno(pt.iso));
